...
func buildTLSConfig(cfg *config.Config, logger *slog.Logger) (*tls.Config, error) {
    ...
    if err != nil {
        // Если не удалось загрузить системный пул, создаём новый.
    ...
    // 3. Добавляем CA из директории (все .pem .crt .cer)
    if cfg.TLSCACertDir != "" {
    ...
    tlsCfg := &tls.Config{
        ...
        ServerName:         strings.Split(cfg.IMAPServer, ":")[0], // SNI
    }
...
func (c *Client) EnsureConnected() error {
    ...
    cl, err := client.NewClient(conn, conn.RemoteAddr().String())
    if err != nil {
        conn.Close()
